...
TrafficGenerator::TrafficGenerator(sc_module_name name, sc_time interval, LocalityType locality, bool do_reads, bool do_writes, unsigned char databyte_value, unsigned int num_transactions)
    : sc_module(name), m_interval(interval), m_locality_type(locality), m_do_reads(do_reads), m_do_writes(do_writes), m_databyte_value(databyte_value), m_num_transactions(num_transactions), m_current_address(0),
